---
id: test-1
lang: pt-BR
book: core
chapterNumber: 0
chapterTitle: Capítulo Teste
pageTitle: Teste 1
slug: teste/teste-1
order: 10
summary: Capítulo de placeholder usado para validar o carregamento de conteúdo.
audience: all
status: draft
---

# Capítulo 1

Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed non risus at
ipsum posuere vulputate.

## Amostra de renderizacao de figura

![Luz permanente sobre uma arcologia selada](/images/landing-hero.png)
_Legenda: Arte ja registrada do projeto reutilizada para validar renderizacao semantica de figuras no conteudo dos capitulos._

## Amostra de renderizacao de tabela

| Sinal                  | Comportamento do reader                                 | Observacoes                                                                                |
| ---------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| Tabela Markdown padrao | Renderiza pela superficie MDX compartilhada do reader   | Autores nao precisam escrever JSX para tabelas comuns.                                     |
| Restricao tatica longa | Mantem a tabela legivel sem quebrar a coluna de leitura | Esta celula intencionalmente longa ajuda a validar overflow horizontal em telas estreitas. |

## Amostra de renderizacao de colunas

:::columns
:::column
Esta coluna mantem a autoria em Markdown comum.

- Suporta paragrafos e listas.
- Preserva a ordem de leitura do arquivo.

:::
:::column
| Checagem | Resultado |
| -------- | --------- |
| Tela larga | As colunas aparecem lado a lado. |
| Tela estreita | As colunas empilham na ordem do arquivo. |
:::
:::

## Amostra de renderizacao de chamadas

:::note[Nota]
Use chamadas de nota para contexto de apoio que precisa ficar separado do fluxo comum do texto.
:::

:::warning[Aviso]
Use chamadas de aviso quando a pessoa leitora deve prestar atencao antes de aplicar uma regra ou procedimento.
:::

:::example[Exemplo]
Chamadas de exemplo podem mostrar uma aplicacao curta de regra sem exigir JSX customizado de autores.
:::

É pra ser isso em português.

## Amostra de renderização de blocos por audiência

Os blocos abaixo validam a filtragem por preferência de leitura. Isso é comportamento de apresentação, não conteúdo protegido.

:::audience{target="player"}
Este bloco voltado para jogadores pode conter Markdown comum para regras, lembretes ou orientação de leitura pensada para o modo jogador.

- Continua legível no arquivo-fonte.
- Usa a preferência de audiência compartilhada.
- Não exige JSX.
  :::

:::audience{target="gm"}
Este bloco voltado para mestre pode conter notas de preparação, lembretes de adjudicação ou orientação de bastidor pensada para o modo GM.

- É filtrado pela preferência de leitura.
- Não é controle de acesso.
- Não é conteúdo secreto ou protegido.
  :::
